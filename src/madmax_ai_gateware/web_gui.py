...
class WebGuiHandler(BaseHTTPRequestHandler):
    ...
    def _state(self) -> dict[str, Any]:
        return {
            ...
            "default_output": display_path(self.config.description_json_path),
        }
...
def _copy_paste_command(cfg: ExperimentConfig) -> str:
    json_path = cfg.description_json_path
    settings_path = cfg.output_dir / "entangler_settings.toml"
    return (
        f"cd {shlex.quote(str(WORKSPACE_ROOT))} && "
        f"./gateware_build/scripts/build_from_json.sh {shlex.quote(display_path(json_path))} "
