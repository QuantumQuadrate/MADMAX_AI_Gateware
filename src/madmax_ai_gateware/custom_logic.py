...
REQUEST_DIR = WORKSPACE_ROOT / "gateware_build" / "ai" / "requests"
CONFIG_EXPERIMENT_DIR = WORKSPACE_ROOT / "gateware_build" / "configs" / "experiments"
...
def verification_commands(slug: str, *, include_nix: bool = False) -> list[str]:
    ...
    config = f"gateware_build/configs/experiments/{safe_slug}.yaml"
    commands = [
...
def _render_codex_prompt(spec: dict[str, Any]) -> str:
    return textwrap.dedent(
        ...
        - Keep the generated experiment config in `gateware_build/configs/experiments/{spec['slug']}.yaml`.
    ...
    config_path.write_text(
        textwrap.dedent(
            ...
            build:
              ...
              description_json: gateware_build/descriptions/{slug}.json
