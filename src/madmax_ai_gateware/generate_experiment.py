...
from .artiq_description import logic_mode_for_branch
...
def _render_experiment(cfg: ExperimentConfig) -> str:
    if cfg.target.variant == "and_nand_test" or cfg.experiment.name == "and_nand_test":
        return _render_and_nand_test_experiment(cfg)
    if (
        cfg.target.variant in {"atom_photon_parity_6", "entangler_atom_photon"}
        or cfg.experiment.name.startswith("atom_photon_parity")
        or logic_mode_for_branch(cfg.repositories.entangler_core_branch) == "atom_photon_parity"
    ):
...
def _render_and_nand_test_experiment(cfg: ExperimentConfig) -> str:
    return f'''# Generated smoke test for {cfg.experiment.name}.
# Source of truth: experiment YAML.
...
class AndNandTestSmoke(EnvExperiment):
    ...
    def run(self):
        self.core.reset()
        self.entangler.init()
        self.entangler.set_run_length_mu({cfg.entangler.timeout_mu} * ns)
        self.entangler.set_timer_div(2)
        _, status = self.entangler.start()
        print(
            "and_nand_test",
            status,
            self.entangler.get_difference_mu(0),
            self.entangler.get_difference_mu(1),
        )
'''
