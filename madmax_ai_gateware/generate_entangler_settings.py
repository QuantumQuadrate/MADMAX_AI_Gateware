...
def generate_entangler_settings(cfg: ExperimentConfig, output: str | Path | None = None) -> Path:
    output_path = (
        resolve_workspace_path(output)
        if output
        else DEFAULT_GENERATED_DIR / "entangler_settings.toml"
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)

    env = Environment(loader=FileSystemLoader(TEMPLATE_DIR), autoescape=False, keep_trailing_newline=True)
    template = env.get_template("entangler_settings.toml.j2")
